...
async function workbookSelected(e) {
	...
	input__workbookName.innerText = WorkBook.name;
	input__workbookName.dataset.value = WorkBook.name;
	input__workbookName.dataset.tempValue = WorkBook.name;
	// app.hierTables();
	Draw.checkResizeSVG();
...
async function checkTablesStorage() {
	...
	if (urls.length !== 0) {
		...
		console.log('Tabelle salvate in session storage');
		App.closeConsole();
	}
}
...
// TODO: 09.05.2025 Potrei usare un file module per includere questo tipo di funzioni
// Un altra Fn simile è getDatabaseTable() utilizzata in supportFn.js
