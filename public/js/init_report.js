...
//            Query.select = { field: Query.field, SQLReport: textarea, alias : alias.value };
	...
	app.setFilteredMetrics = () => {
		...
		document.querySelectorAll("#ul-exist-metrics .selectable[data-selected][data-metric-type='2']").forEach( metricRef => {
			...
			StorageMetric.selected.formula.filters.forEach( filterToken => {
				...
				if (StorageFilter.selected.hasOwnProperty('hier')) {
					// filtro appartenente a un livello dimensionale
    ...
    app.setCompositeMetrics = () => {
        ...
        document.querySelectorAll("#ul-exist-metrics .selectable[data-selected][data-metric-type='4']").forEach( metricRef => {
            StorageMetric.selected = metricRef.dataset.metricToken;
	...
	app.btnSaveReportDone.onclick = () => {
		...
		// metriche filtrate
		app.setFilteredMetrics();
        // metriche composte di base con filtri
        app.setCompositeFilteredMetrics();
        // metriche composte
        app.setCompositeMetrics();
	...
	app.btnSetMetricFilter.onclick = () => {
		...
		console.log('cubi selezionati : ', Query.cubes);
		Query.cubes.forEach( token => {
			StorageCube.selected = token;
		...
		Query.dimensions.forEach( token => {
			StorageDimension.selected = token;
			// per ogni gerarchia
			for (const [hierToken, hier] of Object.entries(StorageDimension.selected.hierarchies)) {
				// per ogni tabella nella gerarchia
				for (const [tableId, table] of Object.entries(StorageDimension.selected.hierarchies[hierToken].order)) {
					const filters = StorageFilter.getFiltersByDimension(token, hierToken, table.table);
					if (filters.size > 0) {
						// debugger;
						filters.forEach( filter => {
							const contentElement = app.tmplList.content.cloneNode(true);
